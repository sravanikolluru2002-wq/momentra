    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.bg }]}>
      ...
      >
        <View style={styles.priceRow}>
          <View>
            <Text style={[styles.price, { color: theme.gold }]}>{formatINR(experience.price)}</Text>
            <Text style={[styles.muted, { color: theme.text2 }]}>
              for {experience.capacity} people
            </Text>
          </View>
          <View style={[styles.duration, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.muted, { color: theme.text2 }]}>⏱ 2 hours</Text>
          </View>
        </View>
        ...
        <SectionTitle title="Momentra Promise" />
        <View style={[styles.promiseCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          {[
            "Setup and booking details are confirmed before your celebration",
            "Momentra coordinates venue, decor, food timing, and add-ons",
            "Support is available during every booked experience",
            "If a promised setup cannot be delivered, we help with a refund or alternate arrangement",
          ].map((item) => (
            <View key={item} style={styles.promiseRow}>
              <Text style={[styles.promiseIcon, { color: theme.gold }]}>✓</Text>
              <Text style={[styles.promiseText, { color: theme.text2 }]}>{item}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  promiseCard: { borderRadius: 14, borderWidth: 1, padding: 12 },
  promiseRow: { alignItems: "flex-start", flexDirection: "row", gap: 9, paddingVertical: 5 },
  promiseIcon: { fontSize: 12, fontWeight: "900", lineHeight: 18 },
  promiseText: { flex: 1, fontSize: 11.5, lineHeight: 18 },
